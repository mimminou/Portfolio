<script>
    ...
    let HeroSection, SimpleSection, ComplexSection, LanguagesSection, ToolsSection, ProjectsSection, ContactSection
    let appSections = []
    let currentElement
    ...
    function scrollTo(direction) {
        if(!isNaN(direction)){
            ...
            scrollIntoView(appSections[currentElementIndex], {cancellable : false}, ()=>{
                scrollingState = true
            })
            currentElement = appSections.at(currentElementIndex)
        }    
        else{
            if (direction === "DOWN" && currentElementIndex < appSections.length - 1){
                scrollingState = false
...
<main class="mainContainer">
    <Navbar NavBarButtons={appSections} on:navClicked={navClicked} bind:visibleSection={currentElementIndex}/>
...
.mainContainer::-webkit-scrollbar { /* WebKit */
    width: 0;
    height: 0;
}
